...
$sql = "SELECT * FROM works WHERE s_id='".$s_id."' AND m_id='".$m_id."' ORDER BY w_id DESC";
...
if(mysql_num_rows($result)==0){
	//新增點擊記錄
	$sql_insert = "INSERT INTO popularity (s_id, w_id, pop_date) VALUES ('".$pop_sid."', '".$w_id."', '".$now_date." ".$now_time."')";
	$result_insert = mysql_query($sql_insert,$pa);
	if(!$result_insert)die("執行SQL命令失敗_insert");
	
	//作品點擊數+1
	$pop_point += 1;
	$sql_update = "UPDATE works SET pop_point='".$pop_point."' WHERE w_id ='".$w_id."'";
