...
<?php
		...
		if(mysql_num_rows($result)!=0){
			//$next = "<a href='showwork.php?mid=".$m_id."&sid=".$next_sid."'>[評上一位 (".$next_sname.") ]</a>　";
			$next = "<a href='showwork.php?mid=".$m_id."&sid=".$next_sid."'>[ 上一位 ]</a>　";
		}
		echo $next;
		$next="";
		...
		$next_sname = $row["s_name"];
		if(mysql_num_rows($result)!=0){
			//$next = "<a href='showwork.php?mid=".$m_id."&sid=".$next_sid."'>[ 評下一位 (".$next_sname.") ]</a>";
			$next = "<a href='showwork.php?mid=".$m_id."&sid=".$next_sid."'>[ 下一位 (".$next_sname.") ]</a>";
		}
		echo $next;
		$next="";
?>		
...
<div align="center">
	<form action="tasave.php" method="post" enctype="multipart/form-data" name="presult" id="presult">
	...
	if(mysql_num_rows($result)!=0){
		...
		elseif (substr($row["w_desc"],-4) == ".mp4"||substr($row["w_desc"],-4) == "webm"||substr($row["w_desc"],-4) == ".ogg"){
			echo '
				...
				<source src="./stu'.$row["w_desc"].'" type="video/'.str_replace('.','',strrchr($row["w_desc"], ".")).'">
		...
		if(mysql_num_rows($result_s)!=0 || mysql_num_rows($result_t)!=0){ //已有互評項目
			...
			while($row_s = mysql_fetch_assoc($result_s)){
				...
				for($k=0;$k<$row_s["sca_n"];$k++){
					...
					//檢查是否已經填寫過。
					$sql_scarid = "SELECT sca_reply FROM scaletr WHERE t_id='".$_SESSION["t_id"]."' AND pg_sid='".$s_id."' AND sca_id='".$row_s["sca_id"]."'";
			...
			while($row_t = mysql_fetch_assoc($result_t)){
				//檢查是否已經填寫過。
				$sql_txtrid = "SELECT txt_reply FROM texttr WHERE t_id='".$_SESSION["t_id"]."' AND pg_sid='".$s_id."' AND txt_id='".$row_t["txt_id"]."'";
				...
				echo "
						...
						<textarea cols=80 rows=3 name='txt".$j."'>".$txtreply."</textarea></br> 
						<input type='submit' name='savesample' value='存成[通用]的評語範例' onclick=\"switchpage('tasave.php?sample=gsave')\" >
						<input type='submit' name='savesample' value='存成[此任務]的評語範例' onclick=\"switchpage('tasave.php?sample=msave')\" >
						</td></tr>";
				echo '
					<script>
						function popText() {
			...
			<input 
				...
				onclick="switchpage(\'tasave.php?fin=done\')" >
			</p>
			
			<!-- 
			<div class="ui-body ui-body-b">
				<fieldset class="ui-grid-a">
					<div class="ui-block-a"><input type="submit" data-theme="a" onclick="switchpage(\'tasave.php?wid='.$w_id.'\')" value="儲存" name="save" style="font-size: 12 pt; border-style: ridge; border-width:3 "></div>
					<div class="ui-block-b"><input type="submit" data-theme="b" onclick="switchpage(\'tasave.php?wid='.$w_id.'&fin=done\')" value="送出" name="submit" style="font-size: 12 pt; border-style: ridge; border-width:3 "></div>
				</fieldset>
			</div>
			-->
			';
		...
		$sql = "SELECT * FROM works WHERE s_id='".$s_id."' AND m_id='".$m_id."' AND w_status='3'";
		$result = mysql_query($sql,$pa);
		if(!$result)die("執行SQL命令失敗ws3");
		$row = mysql_fetch_assoc($result);
		if(mysql_num_rows($result)!=0){
			echo "<font size='4'>作品被退件，尚未補交</font>";
		}
		else{
			echo "<font size='4'>作品尚未繳交</font>";
		}
	}
	$m_name="";
	...
	<form name="groovyform">
				<input type="button" value="回評分頁" onClick="self.parent.location='./pclass.php?cid=<?=$c_id;?>&mid=<?=$m_id;?>&cname=<?=$c_name;?>&mname=<?=$m_name;?>&grade=<?=$m_grade;?>'" style="font-size: 12 pt; border-style: ridge; border-width:3 ">
	</form>
