...
if(isset($_POST["h_id"])){
	...
	if($_POST["action"]=="del"){
		$status = '2'; 
		$sql = "DELETE FROM help WHERE h_id='".$_POST["h_id"]."'";
		$result = mysql_query($sql,$pa);
		if(!$result)die("執行SQL命令失敗");
		else{
			echo "<script language='javascript'>";
			echo "  alert('刪除成功！');";
			echo "document.location.href='helpset.php';";
			echo "</script>";
		}
	}
}
elseif(isset($_POST["action"]) && $_POST["action"]=="add"){
	...
	$status = '5'; 
	for($i=0;$i<$_POST["n"];$i++){
		$sql = "INSERT INTO help (h_word) VALUES ('".$_POST["h_word".$i]."') ";
		$result = mysql_query($sql,$pa);
	}
	if(!$result)die("執行SQL命令失敗");
	else{
		...
		if($newm=="y"){
			echo "document.location.href='tabooset.php?newm=y';";
		}
		else{
...
?>
        <form action="helpset.php<?php echo $newm_get;?>" method="post" enctype="multipart/form-data">
        <table width="900">
<?php
	if($status=='1')	{
		echo "<tr align='center'><th>回饋輔語</th><td></td></tr>";
		//取任務清單
		$sql = "SELECT h_id,h_word FROM help";
		$result = mysql_query($sql,$pa);
		if(!$result)die("執行SQL命令失敗2");
		//取任務數量，來調整空白列的數量
		$num_rows = mysql_num_rows($result);
		while($row = mysql_fetch_assoc($result)){
			echo "<tr><form action='helpset.php' method='post' enctype='multipart/form-data'>";
			echo "<td class='td-solid'>".$row["h_word"]."</td>";
			echo "<td>
		...
		echo "<td class='td-solid'>
				<input type=\"button\" value=\"新增輔語\" onClick=\"self.location='helpset.php?action=form'\">
				</td>";
		echo "</tr>";
	}
	if($status=='3')	{
		echo "<tr align='center'><th>請選擇新增回饋輔語數量</th></tr>";
		echo "<tr>";
		echo "<td class='td-solid'>
	...
	if($status=='4')	{
		echo "<tr><th>回饋輔語</th></tr>";
		for($i=0;$i<$_POST["n"];$i++){
			echo "<tr><td align='center'><input type='text' name='h_word".$i."' id='h_word".$i."'' placeholder='請輸入輔語...'></td></tr>";
		}
		echo "<tr>";
