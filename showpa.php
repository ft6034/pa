...
<tr>
	...
	<tr>
		<td class="title"><?php echo $s_name;?><font size="4">的評審內容</font></td>
		<td>
...
<?php
		...
		if(mysql_num_rows($result)!=0){
			//$next = "<a href='showpa.php?mid=".$m_id."&sid=".$next_sid."'>[評上一位 (".$next_sname.") ]</a>　";
			$next = "<a href='showpa.php?mid=".$m_id."&sid=".$next_sid."'><b>[ 上一位 ]</b></a>　";
		}
		echo $next;
		$next="";
?>
<?php
		...
		$next_sname = $row["s_name"];
		if(mysql_num_rows($result)!=0){
			$next = "<a href='showpa.php?mid=".$m_id."&sid=".$next_sid."'><b>[ 下一位 (".$next_sname.") ]</b></a>";
		}
		echo $next;
		$next="";
?>
...
if($s_num==""){
	...
	echo "<p>第".($s_num+1)."件作品-".$alls_id[$s_num];
	echo "<font color='red'>";
	switch ($alls_pas[$s_num]){
		case "0":
		echo " (尚未互評)";
		...
		if(mysql_num_rows($result_s)!=0 || mysql_num_rows($result_t)!=0){ //已有互評項目
			...
			while($row_s = mysql_fetch_assoc($result_s)){
				...
				for($k=0;$k<$row_s["sca_n"];$k++){
					...
					//檢查是否已經填寫過。
					$sql_scarid = "SELECT sca_reply FROM scaler WHERE s_id='".$s_id."' AND m_id='".$m_id."' AND pg_sid='".$alls_id[$s_num]."' AND sca_id='".$row_s["sca_id"]."'";
			...
			while($row_t = mysql_fetch_assoc($result_t)){
				//檢查是否已經填寫過。
				$sql_txtrid = "SELECT txt_reply FROM textr WHERE s_id='".$s_id."' AND m_id='".$m_id."' AND pg_sid='".$alls_id[$s_num]."' AND txt_id='".$row_t["txt_id"]."'";
