...
<tr>
	...
	<tr>
		<td class="title"><font size="4"><?php echo $row["s_name"];?><font size="2"> [<a href="../index.php" target="parent"> 首頁 </a>] [<a href="../logout.php" target="parent"> 登出 </a>]</font></td>
...
<tr>
	...
	<?php
	//判斷是否已經完成自評
	$sql = "SELECT * FROM works WHERE s_id='".$_COOKIE['s_id']."' AND m_id='".$m_id."' AND w_status='2'";
	$result = mysql_query($sql,$pa);
	if(!$result)die("執行SQL命令失敗1");
	$row = mysql_fetch_assoc($result);
	$w_id = $row["w_id"];
	...
	if(mysql_num_rows($result)!=0){
		...
		if(mysql_num_rows($result_s)!=0 || mysql_num_rows($result_t)!=0){ //已有互評項目
			...
			echo '		
				...
				<input type="hidden" name="pg_sid" value="'.$_COOKIE['s_id'].'">
				';
			...
			while($row_s = mysql_fetch_assoc($result_s)){
				...
				for($k=0;$k<$row_s["sca_n"];$k++){
					...
					//檢查是否已經填寫過。
					$sql_scarid = "SELECT sca_reply FROM scaler WHERE s_id='".$_COOKIE['s_id']."' AND m_id='".$m_id."' AND pg_sid='".$_COOKIE['s_id']."' AND sca_id='".$row_s["sca_id"]."'";
			...
			while($row_t = mysql_fetch_assoc($result_t)){
				//檢查是否已經填寫過。
				$sql_txtrid = "SELECT txt_reply FROM textr WHERE s_id='".$_COOKIE['s_id']."' AND m_id='".$m_id."' AND pg_sid='".$_COOKIE['s_id']."' AND txt_id='".$row_t["txt_id"]."'";
